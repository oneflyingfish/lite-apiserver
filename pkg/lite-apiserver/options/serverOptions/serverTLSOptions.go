...
func (opt *ServerTLSKeyPair) LoadFromConfig(configPath *string) error {
	if configPath == nil || len(*configPath) <= 0 {
		err := "loss config for X.509 Certificate information for lite-apiserver ,you can set by \"--ca-tls-configpath=$path\""
		klog.Error(err)
...
func (opt *ServerTLSKeyPair) validate() error {
	...
	if err := FileExist(opt.CACertPath); err != nil {
		klog.Error("Validate lite-apiserver TLS config: ", err.Error())
		e = err
	}
	...
	if err != nil {
		klog.Error("Validate lite-apiserver TLS pair for certificate and key: ", err.Error())
		return err
	}
